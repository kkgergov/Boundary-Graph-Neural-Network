...
simDir=os.path.join("/home/kgergov/TUM/IDP/BGNN/drum/")
runDir=os.path.join(simDir, "runs1")

NUM_SIMULATIONS=1

nrSimRunsStart=0
nrSimRuns=NUM_SIMULATIONS
...
import pymp
with pymp.Parallel(NUM_SIMULATIONS) as parproc:
 for simInd in parproc.range(nrSimRunsStart, nrSimRunsStop):
  ...
  #for j in range(0, 100):
    ...
    x_DataArr.append(x_Data[sortOrder,:]) #yes
    id_DataArr.append(id_Data[sortOrder]) #yes
  ...
  xData=np.array(x_DataArr)             # X Y Z coordinates of each particle
  idData=np.array(id_DataArr)           # Id of each particle
  typeData=np.array(type_DataArr)       # Type of each particle. Contains seed for the particle generation, radius, etc.
  radiusData=np.array(radius_DataArr)   # Radius of each particle(sphere assumed)
  densityData=np.array(density_DataArr) # Material density of each particle
